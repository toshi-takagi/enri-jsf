...
class ImageLabelingApp:
    def __init__(self, root, image_dir, file_pattern):
        ...
        self.label = tk.Label(self.root, text="Press 'g' for Good, 'b' for Bad, 'e' to Quit")
    ...
    def load_existing_results(self):
        if os.path.exists("status_vel.txt"):
            with open("status_vel.txt", "r") as file:
    ...
    def save_results_to_file(self):
        with open("status_vel.txt", "w") as file:
    ...
    def show_next_image(self):
        if self.current_image_index < len(self.image_files):
            ...
            self.label.configure(text=f"Press 'g' for Good, 'b' for Bad, 'e' to Quit\nCurrent Image: {filename}")
        else:
...
def main():
    image_directory = "results/figs/"
    file_pattern = "output_aatr_vel*.png"
...
if __name__ == "__main__":
    main()
    print('Results are stored in status_vel.txt')
